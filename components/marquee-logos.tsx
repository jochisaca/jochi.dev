...
type Props = { speed?: number } // seconds per full loop
...
export function MarqueeLogos({ speed = 20 }: Props) {
  ...
  return (
    <div className="relative w-full overflow-hidden bg-[#CDEAF2] py-6">
      {/* edge fade to avoid the “cut” look on mobile */}
      <div className="pointer-events-none absolute inset-0 z-10 [mask-image:linear-gradient(to_right,transparent,black_24px,black_calc(100%-24px),transparent)] [-webkit-mask-image:linear-gradient(to_right,transparent,black_24px,black_calc(100%-24px),transparent)]" />
